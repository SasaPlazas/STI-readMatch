...
const STORIES = [
  {
    id: "dark",
    label: "Dark Academia",
    gradient: ["#2B1B69", "#16102E"],
    ink: colors.lime,
    mood: "velvet · ivy · candle",
  },
  {
    id: "cozy",
    label: "Cozy Fantasy",
    gradient: ["#FF7E6B", "#FBF6EB"],
    ink: colors.ink,
    mood: "tea · hearth · familiar",
  },
  {
    id: "psyc",
    label: "Psychological",
    gradient: ["#7C5BFF", "#E8E0FF"],
    ink: colors.ink,
    mood: "unreliable · interior",
  },
  {
    id: "emo",
    label: "Emotional Narratives",
    gradient: ["#FF7E6B", "#7C5BFF"],
    ink: colors.cream,
    mood: "heart · ache · home",
  },
  {
    id: "phi",
    label: "Philosophical",
    gradient: ["#16102E", "#3A2F5C"],
    ink: colors.lime,
    mood: "idea · paradox · slow",
  },
  {
    id: "thr",
    label: "Fast Thrillers",
    gradient: ["#D4FF3D", "#94B82A"],
    ink: colors.ink,
    mood: "pulse · cliff · grip",
  },
  {
    id: "sci",
    label: "Sci-Fi Worlds",
    gradient: ["#7C5BFF", "#16102E"],
    ink: colors.lime,
    mood: "orbit · code · soft",
  },
  {
    id: "char",
    label: "Character-driven",
    gradient: ["#FBF6EB", "#F0E6D2"],
    ink: colors.ink,
    mood: "voice · arc · life",
  },
];
...
export function OnbIdentityScreen({ navigation }) {
  const [pickedStories, setPickedStories] = useState(
    () => new Set(["dark", "psyc", "sci"]),
  );
  ...
  function setRating(authorIdx, star) {
    setAuthorRatings((prev) => ({
      ...prev,
      [authorIdx]: prev[authorIdx] === star ? 0 : star,
    }));
  }
          ...
          onPress={async () => {
            ...
            try {
              const selectedGenres = GENRES.filter((g) =>
                pickedGenres.has(g.id),
              ).map((g) => g.label);
              const selectedAuthors = AUTHORS.filter(
                (_, i) => (authorRatings[i] ?? 0) > 0,
              ).map((a) => a.name);

              try {
                await upsertUserPreferences({
                  favorite_genres: selectedGenres,
                  favorite_authors: selectedAuthors,
                });
              } catch (preferenceError) {
                const message = preferenceError?.message ?? "";
                const isConflict =
            ...
            if (shouldNavigate) {
              navigation.navigate(routes.OnbBehavior);
            }
          }}
        />
      }
      ...
      <View style={styles.progress}>
        <View style={styles.dots}>
          {[1, 2, 3, 4].map((i) => (
            <View
              key={i}
              style={[styles.dot, i <= 1 ? styles.dotOn : styles.dotOff]}
            />
          ))}
        </View>
            ...
            return (
              <Pressable key={s.id} onPress={() => toggleStory(s.id)}>
                ...
                >
                  ...
                  <Text style={[styles.storyLabel, { color: s.ink }]}>
                    {s.label}
                  </Text>
                  <Text style={[styles.storyMood, { color: s.ink }]}>
                    {s.mood}
                  </Text>
                </LinearGradient>
              </Pressable>
        ...
        <View style={styles.genreChips}>
          {GENRES.map((g, i) => {
            ...
            return (
              ...
              >
                <Text
                  style={[styles.genreChipText, on && { fontWeight: "900" }]}
                >
                  {g.label}
                </Text>
              </Pressable>
      ...
      <View style={styles.section}>
        ...
        <View style={styles.authorList}>
          {AUTHORS.map((a, i) => (
            <View
              key={a.name}
              style={[styles.authorRow, a.dark && styles.authorRowDark]}
            >
              <View style={[styles.authorAvatar, { backgroundColor: a.color }]}>
                <Text
                  style={[
                    styles.authorAvatarText,
                    { color: a.dark ? colors.lime : colors.ink },
                  ]}
                >
                  {a.name.charAt(0)}
                </Text>
              ...
              <View style={styles.authorInfo}>
                <Text
                  style={[styles.authorName, a.dark && { color: colors.cream }]}
                >
                  {a.name}
                </Text>
                <Text
                  style={[
                    styles.authorTag,
                    a.dark && { color: "rgba(251,246,235,0.6)" },
                  ]}
                >
                  {a.tag}
                </Text>
                  ...
                  return (
                    ...
                    >
                      <Text
                        style={[
                          styles.starText,
                          { color: filled ? colors.ink : "rgba(22,16,46,0.3)" },
                        ]}
                      >
                        ★
                      </Text>
