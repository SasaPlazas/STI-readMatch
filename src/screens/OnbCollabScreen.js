...
export function OnbCollabScreen({ navigation }) {
  ...
  return (
    <Screen
      ...
      footer={<RMButton title="Reveal my reader aura ✦" variant="dark" onPress={() => navigation.navigate(routes.OnbReveal)} />}
    >
      ...
      <View style={styles.card}>
        ...
        <View style={styles.nameRow}>
          <View style={[styles.circleIcon, { backgroundColor: selectedVibe?.color ?? colors.purple }]}>
          ...
          {VIBES.map((v) => {
            ...
            return (
              <Pressable
                ...
                style={[styles.vibeChip, on && { backgroundColor: v.color, borderColor: colors.ink, borderWidth: 1.5 }]}
              >
      ...
      <View style={styles.card}>
        ...
        <View style={styles.methodRow}>
          {[
            ...
            <Pressable key={m.label} style={[styles.methodBtn, { backgroundColor: m.bg }]}>
              <Text style={[styles.methodBtnText, { color: m.text }]}>{m.label}</Text>
            </Pressable>
          ))}
        </View>
      </View>
        ...
        <View style={styles.tgRow}>
          ...
          <View style={{ flex: 1 }}>
            ...
            <Text style={[styles.tgBody, tgOn && { color: 'rgba(251,246,235,0.75)' }]}>
              We'll create a private channel. Recs, votes & members sync both ways — automatically.
            </Text>
            {tgOn && (
              <View style={styles.tgPills}>
                <View style={styles.tgPillLime}><Text style={styles.tgPillLimeText}>● Auto-create</Text></View>
                <View style={styles.tgPillGlass}><Text style={styles.tgPillGlassText}>Bidirectional</Text></View>
              </View>
            )}
          </View>
...
const styles = StyleSheet.create({
  ...
  circleIcon: { width: 56, height: 56, borderRadius: 16, alignItems: 'center', justifyContent: 'center', flexShrink: 0 },
  circleInitials: { fontSize: 20, fontWeight: '900', color: colors.ink, letterSpacing: -0.5 },
  nameInput: { flex: 1, fontSize: 22, fontWeight: '900', color: colors.ink, borderBottomWidth: 2, borderBottomColor: colors.purple, paddingBottom: 4, letterSpacing: -0.5 },
  vibes: { flexDirection: 'row', flexWrap: 'wrap', gap: 8 },
  vibeChip: { flexDirection: 'row', alignItems: 'center', gap: 6, borderRadius: radii.pill, paddingVertical: 8, paddingHorizontal: 12, backgroundColor: colors.mist, borderWidth: 1, borderColor: 'transparent' },
  ...
  inviteName: { fontSize: 8, fontWeight: '800', letterSpacing: 0.8, color: 'rgba(22,16,46,0.55)', textTransform: 'uppercase' },
  inviteEmpty: { width: 48, height: 48, borderRadius: 24, borderWidth: 1.5, borderStyle: 'dashed', borderColor: 'rgba(22,16,46,0.25)', alignItems: 'center', justifyContent: 'center' },
  inviteEmptyText: { fontSize: 20, fontWeight: '900', color: 'rgba(22,16,46,0.4)' },
  methodRow: { flexDirection: 'row', gap: 6 },
  methodBtn: { flex: 1, borderRadius: 12, paddingVertical: 10, alignItems: 'center' },
  ...
  tgPillLime: { borderRadius: radii.pill, paddingVertical: 4, paddingHorizontal: 10, backgroundColor: 'rgba(212,255,61,0.12)' },
  tgPillLimeText: { fontSize: 10, fontWeight: '800', color: colors.lime },
  tgPillGlass: { borderRadius: radii.pill, paddingVertical: 4, paddingHorizontal: 10, backgroundColor: 'rgba(255,255,255,0.08)' },
  tgPillGlassText: { fontSize: 10, fontWeight: '800', color: 'rgba(251,246,235,0.65)' },
  ...
  syncBox: { marginTop: 14, padding: 12, borderRadius: 12, backgroundColor: 'rgba(212,255,61,0.08)', borderWidth: 0.5, borderColor: 'rgba(212,255,61,0.25)' },
  syncLabel: { fontSize: 9, fontWeight: '800', letterSpacing: 1.6, color: 'rgba(251,246,235,0.5)', textTransform: 'uppercase', marginBottom: 8 },
  syncTags: { flexDirection: 'row', flexWrap: 'wrap', gap: 6 },
  syncTag: { borderRadius: radii.pill, paddingVertical: 4, paddingHorizontal: 10, backgroundColor: 'rgba(255,255,255,0.06)' },
  syncTagText: { fontSize: 10, fontWeight: '800', color: colors.lime, letterSpacing: 0.4 },
