...
const SHELVES = [
  ...
  { id: 'now',  label: 'Currently reading', count: 2,   color: colors.coral,    icon: '◐' },
  { id: 'wnt',  label: 'Want to read',      count: 28,  color: colors.purple,   icon: '☆' },
  { id: 'fav',  label: 'Favorites',         count: 18,  color: colors.lavender, icon: '♥' },
  { id: 'drop', label: 'Dropped',           count: 6,   color: colors.ink,      icon: '✕', dark: true },
];
...
const BADGE_LABELS = ['Comfort read', 'Mind-expanding', 'Life‑changing', 'Emotional'];
...
export function OnbBehaviorScreen({ navigation }) {
  ...
  return (
    <Screen
      ...
      footer={<RMButton title="Continue · Personality" variant="dark" onPress={() => navigation.navigate(routes.OnbPersonality)} />}
    >
      ...
      <View style={styles.section}>
        ...
        <View style={styles.list}>
          {top.map((b, i) => (
            <View key={b.id} style={styles.row}>
              ...
              <View style={[styles.badge, { backgroundColor: BADGE_COLORS[i] }]}>
        ...
        <View style={styles.hintBox}>
          <View style={styles.hintIcon}><Text style={styles.hintIconText}>✦</Text></View>
          ...
          {SHELVES.map((s) => (
            ...
            >
              <View style={styles.shelfTop}>
                <View style={[styles.shelfIcon, { backgroundColor: s.dark ? 'rgba(212,255,61,0.15)' : colors.ink }]}>
                  <Text style={[styles.shelfIconText, { color: s.dark ? colors.lime : s.color }]}>{s.icon}</Text>
                </View>
                <Text style={[styles.shelfCount, { color: s.dark ? colors.cream : colors.ink }]}>{s.count}</Text>
              </View>
              <Text style={[styles.shelfLabel, { color: s.dark ? colors.cream : colors.ink }]}>{s.label}</Text>
            </View>
          ))}
        </View>
      </View>
...
const styles = StyleSheet.create({
  ...
  sectionHint: { fontSize: 9, fontWeight: '800', letterSpacing: 1.2, color: 'rgba(22,16,46,0.4)', textTransform: 'uppercase' },
  freqHint: { fontSize: 9, fontWeight: '800', letterSpacing: 1.2, color: 'rgba(22,16,46,0.4)', textTransform: 'uppercase', marginTop: 4, marginBottom: 14 },
  list: { gap: 8 },
  row: { flexDirection: 'row', alignItems: 'center', backgroundColor: colors.white, borderRadius: radii.lg, padding: 12, gap: 12, borderWidth: 1, borderColor: 'rgba(22,16,46,0.06)' },
  rank: { width: 24, textAlign: 'center', fontWeight: '900', color: colors.ink, fontSize: 18 },
  rowText: { flex: 1, minWidth: 0 },
  rowTitle: { fontWeight: '900', color: colors.ink, letterSpacing: -0.2 },
  rowMeta: { marginTop: 2, fontStyle: 'italic', color: 'rgba(22,16,46,0.65)', fontWeight: '600', fontSize: 12 },
  badge: { borderRadius: radii.pill, paddingVertical: 6, paddingHorizontal: 10 },
  badgeText: { fontSize: 10, fontWeight: '800', color: colors.ink, letterSpacing: 0.4 },
  addSlot: { height: 58, borderRadius: radii.lg, borderWidth: 1.5, borderColor: 'rgba(22,16,46,0.18)', borderStyle: 'dashed', alignItems: 'center', justifyContent: 'center', flexDirection: 'row', gap: 8 },
  addIcon: { width: 24, height: 24, borderRadius: 12, backgroundColor: 'rgba(22,16,46,0.08)', alignItems: 'center', justifyContent: 'center' },
  addIconText: { fontSize: 16, fontWeight: '900', color: 'rgba(22,16,46,0.55)' },
  addLabel: { fontSize: 14, fontWeight: '700', color: 'rgba(22,16,46,0.55)' },
  hintBox: { marginTop: 10, padding: 12, borderRadius: radii.md, backgroundColor: 'rgba(124,91,255,0.08)', flexDirection: 'row', alignItems: 'center', gap: 10 },
  hintIcon: { width: 26, height: 26, borderRadius: 8, backgroundColor: colors.purple, alignItems: 'center', justifyContent: 'center', flexShrink: 0 },
  ...
  shelfCard: { width: '47.5%', borderRadius: radii.lg, padding: 14, borderWidth: 1.5, borderColor: colors.ink },
  shelfCardFull: { width: '100%' },
  shelfTop: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 },
  shelfIcon: { width: 28, height: 28, borderRadius: 9, alignItems: 'center', justifyContent: 'center' },
  ...
  freqCard: { width: '47.5%', backgroundColor: colors.white, borderRadius: radii.lg, padding: 14, borderWidth: 1, borderColor: 'rgba(22,16,46,0.08)' },
  freqCardActive: { backgroundColor: colors.lime, borderColor: colors.ink, borderWidth: 1.5 },
  freqIconWrap: { width: 32, height: 32, borderRadius: 10, alignItems: 'center', justifyContent: 'center', marginBottom: 10 },
